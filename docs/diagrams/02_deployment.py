...
dep.connect("tauri_proc",   "prisma_proc",   label="WebView → :8765/app")
dep.connect("prisma_proc",  "vault_fs",      label="read / write")
dep.connect("prisma_proc",  "chromadb_fs",   label="upsert / query")
dep.connect("prisma_proc",  "graphify_fs",   label="read / write")
dep.connect("prisma_proc",  "ui_build_fs",   label="serves /app")
dep.connect("prisma_proc",  "ollama_win",    label="HTTP (eth0 bridge)")
dep.connect("prisma_proc",  "zotero_win",    label="HTTP (eth0 bridge)")
dep.connect("tauri_proc",   "explorer_win",  label="open_url → explorer.exe")
...
net.connect("tauri_webview", "prisma_api",    label=":8765/app (GET)")
net.connect("pwa_client",    "prisma_api",    label=":8765/app (GET)")
net.connect("tauri_webview", "dev_reload",    label="poll :8765/ui/dev/version", style="dashed")
net.connect("pwa_client",    "dev_reload",    label="poll :8765/ui/dev/version", style="dashed")
net.connect("prisma_api",    "ollama_ep",     label="HTTP POST /api/generate")
net.connect("prisma_api",    "zotero_ep",     label="HTTP GET /api/")
net.connect("tauri_webview", "url_open_ep",   label="Tauri command open_url")
net.connect("prisma_api",    "zotero_web_ep", label="HTTPS (Bearer token)")
